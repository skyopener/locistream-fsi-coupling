...
//          cout << "scalar constant displacement specification" << endl ;
...
//          cout << "vector constant displacement specification" << endl ;
      ...
      void calculate(Entity face) {
        for(const int *ni=face2node.begin(face);ni!=face2node.end(face);++ni){
          node_s_b[*ni]=s_BC[ref[face]].Value(*solutionTime+*timeStep,
            pos[*ni]) ;
//cout << "rank,n,s: " << Loci::MPI_rank << " " << *ni << " " << node_s_b[*ni] << endl ;
        }
      }
