...
namespace streamUns {
  ...
  // Rule for creating constraint for all the edges.
  class EdgeConstraint : public pointwise_rule {
    private:
  ...
  register_rule<EdgeConstraint> registerEdgeConstraint ;

  // Rule for creating a boundary edge constraint.
  class BoundaryEdgeConstraint : public pointwise_rule {
    private:
      const_multiMap face2edge ;
      store<bool> boundaryEdges ;
    public:
  ...
  register_rule<BoundaryEdgeConstraint> registerBoundaryEdgeConstraint ;
...
//-----------------------------------------------------------------------------
// Rules for computing spatially varying value of stiffness parameter chi. This
// parameter is defined for each of the nodes and is based on the nodal/dual
// volume. Chi ranges from the max to the min specified values as cells go from
// smallest to largest. A logarithmic variation is used since the dual volume
// range may be orders of magnitude.

  class MinimumDualVolumeUnit : public unit_rule {
    private:
      param<real> minDualVolume ;
    public:
      ...
      void compute(const sequence &seq) {
        *minDualVolume=std::numeric_limits<real>::max() ;
      }
  } ;

  register_rule<MinimumDualVolumeUnit> registerMinimumDualVolumeUnit ;

  class MinimumDualVolumeApply : public apply_rule<param<real>,
  Loci::Minimum<real> > {
    private:
    ...
    public:
      ...
      void calculate(Entity node) { join(*minDualVolume,dualVolume[node]) ; }

      // Initialize the value.
      void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<MinimumDualVolumeApply> registerMinimumDualVolumeApply ;

  class MaximumDualVolumeUnit : public unit_rule {
    private:
      param<real> maxDualVolume ;
    public:
      ...
      // Initialize the value.
      void compute(const sequence &seq) {
        *maxDualVolume=std::numeric_limits<real>::min() ;
      }
  } ;

  register_rule<MaximumDualVolumeUnit> registerMaximumDualVolumeUnit ;

  class MaximumDualVolumeApply : public apply_rule<param<real>,
  Loci::Maximum<real> > {
    private:
      const_store<real> dualVolume ;
      param<real> maxDualVolume ;
    public:
      ...
      // Update maximum for a node.
      void calculate(Entity node) { join(*maxDualVolume,dualVolume[node]) ; }

      // Loop over nodes.
      void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<MaximumDualVolumeApply> registerMaximumDualVolumeApply ;

  // Default for chi from user input.
  class DefaultChi : public pointwise_rule {
    private:
      const_param<real> gridMoverChi ;
      store<real> chi ;
    public:
      ...
      // Set the edge value.
      void calculate(Entity edge) { chi[edge]=*gridMoverChi ; }

      // Loop over edges.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<DefaultChi> registerDefaultChi ;

  // Compute chi at nodes.
  class NodeChiFromDualVolume : public pointwise_rule {
    private:
      const_param<real> gridMoverMinChi,gridMoverMaxChi ;
      const_param<real> minDualVolume,maxDualVolume ;
      const_store<real> dualVolume ;
      store<float> node_chi ;
    private:
      real a,b ;
    public:
      ...
      void calculate(Entity node) {
        node_chi[node]=a*log10(dualVolume[node])+b ;
      }

      // Compute the interpolation parameters and then loop over edges.
      virtual void compute(const sequence &seq) {
        a=(*gridMoverMaxChi-*gridMoverMinChi)/(log10(*minDualVolume)-
          log10(*maxDualVolume)) ;
        b=*gridMoverMaxChi-a*log10(*minDualVolume) ;
        do_loop(seq,this) ;
      }
  } ;

  register_rule<NodeChiFromDualVolume> registerNodeChiFromDualVolume ;

  // Compute chi using a logarithmic interpolation based on the dual volume.
  class EdgeChi : public pointwise_rule {
    private:
      const_MapVec<2> edge2node ;
      const_store<float> node_chi ;
      store<real> chi ;
    public:
      ...
      void calculate(Entity edge) {
        chi[edge]=0.5*(node_chi[edge2node[edge][0]]+node_chi[edge2node[edge]
          [1]]) ;
      }
  ...
  register_rule<EdgeChi> registerEdgeChi ;
  ...
  <store<vect3d>,Loci::Summation<vect3d> > {
    ...
    public:
      ...
      DilatationToDisplacementSourceTerm() {
        ...
        name_store("sSourceTerm{n,itg}",sSourceTerm) ;
        input("gridMoverNu{n,itg},gridMoverFactor0{n},dualArea{n},scaledDualArea{n}") ;
