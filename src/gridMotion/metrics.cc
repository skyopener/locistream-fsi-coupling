...
namespace streamUns {
  ...
  // Unit rule to initialize cell volume. We have constrained this to only be
  // active for deforming meshes since for rigid-body motions the volumes stay
  // constant and thus differences in volume are identically zero no matter how
  // they are computed. Thus, no sense using an expensive computation. This
  // constraint was added much before this comment. 12/11/2009
  class GridMotionVolumeUnit : public unit_rule {
    ...
    public:
      ...
      GridMotionVolumeUnit() {
        ...
        output("gridMotion::vol") ;
        constraint("geom_cells,gridMover") ;
      }
  ...
  Loci::Summation<real> > {
    ...
    public:
      ...
      GridMotionVolumeApplyInterior() {
        ...
        output("(cl,cr)->gridMotion::vol") ;
        constraint("internalFaces,gridMover") ;
      }
  ...
  Loci::Summation<real> > {
    ...
    public:
      ...
      GridMotionVolumeApplyBoundary() {
        ...
        name_store("gridMotion::vol",vol) ;
        input("face2edge->edge2node->pos,area,facecenter") ;
        output("ci->gridMotion::vol") ;
        constraint("boundaryFaces,gridMover") ;
      }
