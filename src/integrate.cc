  ...
  class WriteVolumeData : public singleton_rule {
    ...
    public:
      ...
      WriteVolumeData() {
        ...
        name_store("totalVolume{n,it}",totalVolume) ;
        name_store("totalMass{n,it}",totalMass) ;
        name_store("totalEnergy{n,it}",totalEnergy) ;
        name_store("totalEnthalpy{n,it}",totalEnthalpy) ;
        name_store("totalSpeciesMass{n,it}",totalSpeciesMass) ;
        name_store("OUTPUT{n,it}",OUTPUT) ;
        input("eos{n,it}") ;
        input("totalVolume{n,it},totalMass{n,it},totalEnergy{n,it}") ;
        input("totalEnthalpy{n,it},totalSpeciesMass{n,it}") ;
        output("OUTPUT{n,it}") ;
...
namespace streamUns {
  ...
  // The parameter specifies the boundary constraint. Note that we are now
  // using a bogus input "thetaParameter" so that the param produced will be
  // defined over "UNIVERSE" rather than just "X" when we had this rule
  // constrained by "X" . This was causing trouble for our new global
  // conservation mass flux rule in momentum.cc .
  class BoundaryMassTransferUnit : public unit_rule {
    private:
      const_param<real> thetaParameter ; // Bogus input.
...
//      constraint("X") ;
  ...
  // Rule to add zero for inviscid flow so we don't get messages that cannot
  // find any apply rules. This prevents us from having to have multiple
  // output rules later.
...
//          << tot_mom.x << ' ' << tot_mom.y << ' ' << tot_mom.z << ' '
  ...
  class PressureForceUnit : public unit_rule {
    ...
    public:
      ...
      // Define input and output.
      PressureForceUnit() {
        name_store("pressureForce{n,it}",pressureForce) ;
        constraint("UNIVERSE{n,it}") ;
        output("pressureForce{n,it}") ;
      }
  ...
  Loci::Summation<vect3d> > {
    ...
    public:
      ...
      // Define input and output.
      PressureForceApply() {
        name_store("p_f{n,it}",p_f) ;
        name_store("area{n}",area) ;
        name_store("pressureForce{n,it}",pressureForce) ;
        input("p_f{n,it},area{n}") ;
        output("pressureForce{n,it}") ;
  ...
  class ViscousForceUnit : public unit_rule {
    ...
    public:
      ...
      // Define input and output.
      ViscousForceUnit() {
        name_store("viscousForce{n,it}",viscousForce) ;
        constraint("noslip_BC{n,it}") ;
        output("viscousForce{n,it}") ;
      }
  ...
  Loci::Summation<vect3d> > {
    ...
    public:
      ...
      ViscousForceWallFunctionApply() {
        ...
        output("viscousForce{n,it}") ;
        constraint("noslip_BC{n,it},ref->wallFunction_BCoption{n,it}") ;
      }
  ...
  Loci::Summation<vect3d> > {
    ...
    public:
      ...
      ViscousForceNoWallFunctionApply() {
        ...
        name_store("noWallFunction",noWallFunction) ;
        name_store("viscousForce{n,it}",viscousForce) ;
        input("ci->gradv3d(v){n,it},laminarViscosity_f{n,it}") ;
...
//  registerViscousMomentNoWallFunctionApply ;
  ...
  class NoSlipOutput : public pointwise_rule {
    ...
    public:
      ...
      NoSlipOutput() {
        name_store("pressureForce{n,it}",pressureForce) ;
        name_store("viscousForce{n,it}",viscousForce) ;
        name_store("energyTransfer(noslip_BC){n,it}",energyTransfer) ;
        name_store("OUTPUT{n,it}",OUTPUT) ;
        input("pressureForce{n,it},viscousForce{n,it}") ;
        input("energyTransfer(noslip_BC){n,it}") ;
        output("OUTPUT{n,it}") ;
        conditional("do_print{n,it}") ;
      }
  ...
  class NoSlipMomentOutput : public pointwise_rule {
    ...
    public:
      ...
      // Define input and output.
      NoSlipMomentOutput() {
        name_store("pressureMoment{n,it}",pressureMoment) ;
        name_store("viscousMoment{n,it}",viscousMoment) ;
        name_store("OUTPUT{n,it}",OUTPUT) ;
        input("pressureMoment{n,it},viscousMoment{n,it}") ;
        output("OUTPUT{n,it}") ;
        conditional("do_print{n,it}") ;
      }
  ...
  // Write out boundary area, as well as mass, momentum and energy transfer
  // data for each boundary condition in vars file.
  class NewBoundaryOutput : public pointwise_rule {
    private:
      const_param<string> boundaryName ;
      const_param<int> nCycle ;
      const_param<real> sTime ;
      const_param<real> totalArea ;
      const_param<real> massTransfer ;
    ...
    public:
      ...
      // Define input and output.
      NewBoundaryOutput() {
        name_store("boundaryName(X){n,it}",boundaryName) ;
        name_store("ncycle{n}",nCycle) ;
        name_store("stime{n}",sTime) ;
        name_store("totalArea(boundaryName(X)){n,it}",totalArea) ;
        name_store("massTransfer(boundaryName(X)){n,it}",massTransfer) ;
        name_store("energyTransfer(boundaryName(X)){n,it}",energyTransfer) ;
        name_store("boundaryPressureForce(boundaryName(X)){n,it}",boundaryPressureForce) ;
        name_store("boundaryViscousForce(boundaryName(X)){n,it}",boundaryViscousForce) ;
        name_store("OUTPUT{n,it}",OUTPUT) ;
        input("boundaryName(X){n,it},ncycle{n},stime{n}") ;
        input("totalArea(boundaryName(X)){n,it}") ;
        input("massTransfer(boundaryName(X)){n,it}") ;
        input("energyTransfer(boundaryName(X)){n,it}") ;
        input("boundaryPressureForce(boundaryName(X)){n,it}") ;
        input("boundaryViscousForce(boundaryName(X)){n,it}") ;
        output("OUTPUT{n,it}") ;
        conditional("do_print{n,it}") ;
        set_parametric_variable("boundaryName(X){n,it}") ;
      }
      ...
      void compute(const sequence &seq) {
        ...
        filename << "output/flux_" << *boundaryName << ".dat" ;
        ofstream out  ;
        if(*nCycle==0) out.open(filename.str().c_str(),ios::out) ;
        else out.open(filename.str().c_str(),ios::app) ;
        if(!out.fail()){
          out.precision(10) ;
          out << *nCycle << " " << *sTime << " " << *massTransfer << " "
//          << tot_mom.x << ' ' << tot_mom.y << ' ' << tot_mom.z << ' '
        ...
        // Write data to standard output.
        cout.setf(ios::scientific,ios::floatfield) ; cout.precision(4) ;
        cout << "Integrated Boundary Data (" << *boundaryName<< ")"
          << endl ;
        cout << "  total area = " << *totalArea << " m^2" << endl ;
        cout << "  mass transfer = " << *massTransfer << " kg/s" << endl ;
        cout << "  energy transfer = " << *energyTransfer << " W" << endl ;
  ...
  register_rule<NewBoundaryOutput> registerNewBoundaryOutput ;
}
