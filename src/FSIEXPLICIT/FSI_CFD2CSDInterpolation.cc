  ...
  class FSIBoundaryFaceUnitRHS : public singleton_rule {  	
    ...
    public:
      ...
      FSIBoundaryFaceUnitRHS() {
        ...
        name_store("fsiBoundaryFaceB(X){n}",b) ;
...
#line 461 "FSI_CFD2CSDInterpolation.loci"
    Loci::const_param<vector<int> >  L_fsiNumBoundaryFaceX_n__ ; 
#line 461 "FSI_CFD2CSDInterpolation.loci"
    Loci::const_blackbox<vector<real> >  L_fsiBoundaryFaceQX_n__ ; 
#line 461 "FSI_CFD2CSDInterpolation.loci"
    Loci::const_blackbox<PETSCCFD2CSDMultiVector>  L_fsiBoundaryFaceBX_n__ ; 
#line 461 "FSI_CFD2CSDInterpolation.loci"
    Loci::blackbox<vector<real> >  L_fsiBoundaryFaceWeightX_n__ ; 
#line 461 "FSI_CFD2CSDInterpolation.loci"
public:
#line 461 "FSI_CFD2CSDInterpolation.loci"
...
#line 461 "FSI_CFD2CSDInterpolation.loci"
       name_store("fsiNumBoundaryFace(X){n}",L_fsiNumBoundaryFaceX_n__) ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       name_store("fsiBoundaryFaceQ(X){n}",L_fsiBoundaryFaceQX_n__) ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       name_store("fsiBoundaryFaceB(X){n}",L_fsiBoundaryFaceBX_n__) ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       name_store("fsiBoundaryFaceWeight(X){n}",L_fsiBoundaryFaceWeightX_n__) ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       input("fsiNumBoundaryFace(X){n},fsiBoundaryFaceQ(X){n},fsiBoundaryFaceB(X){n},FSIRBFr,FSIRBFa,FSIRBFnr,FSIRBFMaxLinearSolverIterations,FSIIterationTolerance") ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       output("fsiBoundaryFaceWeight(X){n}") ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       constraint("X{n}") ;
#line 461 "FSI_CFD2CSDInterpolation.loci"
       disable_threading() ;
...
#line 461 "FSI_CFD2CSDInterpolation.loci"
    ...
    // Get the number of local and global nodes.
        int localNumFace=(*L_fsiNumBoundaryFaceX_n__)[rank],globalNumFace=0 ;
        for(unsigned int i=0;i<(*L_fsiNumBoundaryFaceX_n__).size();++i) globalNumFace+=(*L_fsiNumBoundaryFaceX_n__)[i] ;
    ...
    //    cout << "p, r, localNumface, globalnumFace: "<< p << delim << rank << delim << localNumFace  << delim << globalNumFace << endl ;
		...
		// Compute the row offset for this process.
        int localStart=0 ; 
        for(int i=0;i<rank;++i) localStart+=(*L_fsiNumBoundaryFaceX_n__)[i]  ;  // 3 coordinates
...
//				vector<int> numberOfOffDiagNonZeros(localNumFace+((rank==p-1)?4:0),0) ;
		...
		// Determine the number of nonzero local elements		
		    ...
		    cout << "Start Allocation A: rank = " << rank << endl ; //", " << (*$fsiBoundaryFaceQ(X){n}).size() << endl ;
		  //  for(int i=localStart;i<localEnd;++i) {
		 //   	I=3*i; I1=3*i+1; I2=3*i+2 ;
		  //  	cout << "rank: " << rank << ", i: " << i << "Qx, Qy, Qz: " << (*$fsiBoundaryFaceQ(X){n})[I] << delim << (*$fsiBoundaryFaceQ(X){n})[I1] << delim << (*$fsiBoundaryFaceQ(X){n})[I2] << endl ;
		  //  }
					...
					for(int j=0;j<globalNumFace;++j){		
						...
						if( (j>=localStart) && (j<localEnd) ) {		// DIAGONAL					
							if (fsiNr > 0 && fsiNr < 9) { // compact								
								distance = pow((*L_fsiBoundaryFaceQX_n__)[I] - (*L_fsiBoundaryFaceQX_n__)[J], 2.) ; //)*((*Q)[I] - (*Q)[J]) ;
								distance += pow((*L_fsiBoundaryFaceQX_n__)[I1] - (*L_fsiBoundaryFaceQX_n__)[J1], 2.) ; //*((*Q)[I1] - (*Q)[J1]) ;
								distance += pow((*L_fsiBoundaryFaceQX_n__)[I2] - (*L_fsiBoundaryFaceQX_n__)[J2], 2.) ; //)*((*Q)[I2] - (*Q)[J2]) ;
...
//						numberOfDiagNonZeros[count+i]=globalNumFace-localStart-4;
        ...
        PetscScalar columnValue ;
				...
				//			cout << "columnvalueM = " << columnValue << endl ;
					...
					valueQ[0] = 1.0; valueQ[1]=(*L_fsiBoundaryFaceQX_n__)[I]; valueQ[2]=(*L_fsiBoundaryFaceQX_n__)[I1]; valueQ[3]=(*L_fsiBoundaryFaceQX_n__)[I2];
					for(int j=0; j<4; ++j) {
	...
	//			cout << "Force Interpolation CFD2CSD: p,r: phi created" << Loci::MPI_processes << delim << Loci::MPI_rank << endl ;
		...
		//		cout << "Force Interpolation CFD2CSD: p,r: after operator" << Loci::MPI_processes << delim << Loci::MPI_rank << endl ;
				CFD2CSDKsp_ksp.SolveVector((*L_fsiBoundaryFaceBX_n__),(phi)) ;
				endtime = MPI_Wtime();
		...
		//		CFD2CSD_A_matrix.Destroy() ;
				...
				for(int i=0;i<p;++i) {
					for(int j=0;j<i;++j) displs[i]+=(3*(*L_fsiNumBoundaryFaceX_n__)[j]);
					recvcounts[i] = 3*((i==p-1)?((*L_fsiNumBoundaryFaceX_n__)[i]+4):(*L_fsiNumBoundaryFaceX_n__)[i]) ;
				}
				
				// Allocate fsiQ
				//(*fsiWeight).resize(3*globalNumFace) ; // number of nodes * 3 coordinates
				(*L_fsiBoundaryFaceWeightX_n__).resize(3*globalNumFace) ; // number of nodes * 3 coordinates
				...
				for(int i=localStart; i<localStart+localNumFace; ++i,++counter) {
					(*L_fsiBoundaryFaceWeightX_n__)[3*i+0] = phixCopy[counter] ; //cout << "fsiWeight[3*" << i << "+0]=" << (*fsiWeight)[3*i+0] << ", " ;
					(*L_fsiBoundaryFaceWeightX_n__)[3*i+1] = phiyCopy[counter] ; //cout << "fsiWeight[3*" << i << "+1]=" << (*fsiWeight)[3*i+1] << ", " ;
					(*L_fsiBoundaryFaceWeightX_n__)[3*i+2] = phizCopy[counter] ; //cout << "fsiWeight[3*" << i << "+2]=" << (*fsiWeight)[3*i+2] << endl ;
				}
		        (phi).RestoreArray(&phixCopy,&phiyCopy,&phizCopy) ; // according to manual inexpensive
		
				// Allgatherv
				MPI_Allgatherv(&(*L_fsiBoundaryFaceWeightX_n__)[3*localStart], 3*localNumFace, MPI_DOUBLE, &(*L_fsiBoundaryFaceWeightX_n__)[0], &recvcounts[0], &displs[0], MPI_DOUBLE, MPI_COMM_WORLD);
//				PetscScalar *phixCopy,*phiyCopy,*phizCopy ;
...
//	  const_param<bool> CFDIterationFinished ;
    ...
    public:
      ...
      FSIInternalFaceApply() {
		name_store("fsiBoundaryFaceQ(topCSDfaces){n}",Qtop) ;
		name_store("fsiBoundaryFaceQ(bottomCSDfaces){n}",Qbottom) ;
		name_store("fsiBoundaryFaceWeight(topCSDfaces){n}",fsiWeightTop) ;
		name_store("fsiBoundaryFaceWeight(bottomCSDfaces){n}",fsiWeightBottom) ;
		name_store("FSIRBFr",r) ;
		name_store("FSIRBFa",a) ;
		name_store("FSIRBFnr",fsiNr) ;
    name_store("fsiNumBoundaryFace(topCSDfaces){n}",fsiNumBoundaryFaceTop) ;
    name_store("fsiNumBoundaryFace(bottomCSDfaces){n}",fsiNumBoundaryFaceBottom) ;
    name_store("CSDnodes{n}",CSDnodes_it) ;
	name_store("CSDConnectivity{n}",CSDConnectivity) ;
    name_store("CSDForce{n}",CSDForce) ;
...
//		name_store("CFDIterationFinished{n,it-1}",CFDIterationFinished) ;
 //   input("CFDIterationFinished{n,it-1}") ;
    ...
    input("CSDdimension") ;
		...
		input("fsiNumBoundaryFace(topCSDfaces){n}") ;
    input("fsiNumBoundaryFace(bottomCSDfaces){n}") ;
		input("fsiBoundaryFaceQ(topCSDfaces){n}") ;
		input("fsiBoundaryFaceQ(bottomCSDfaces){n}") ;
		input("fsiBoundaryFaceWeight(topCSDfaces){n}") ;
		input("fsiBoundaryFaceWeight(bottomCSDfaces){n}") ;
		input("CSDnodes{n}") ;
    output("CSDForce{n}") ;
	constraint("FSINLAMS") ;
    constraint("FSICoupling") ; //internal nodes
    constraint("topCSDfaces{n},bottomCSDfaces{n}") ; //internal nodes
  //  disable_threading() ;
      }
