...
namespace streamUns {
  ...
  /* Class to set the restart flag. ORIGINAL 3/18/2008
  class DoRestart : public singleton_rule {
    ...
    public:
      ...
      DoRestart() {
        ...
        name_store("restart_freq{n,it}",restartFrequency) ;
        name_store("do_restart{n,it}",doRestart) ;
        input("$n{n},$it{n,it}") ;
        input("iterationFinished{n,it},restart_freq{n,it}");
        output("do_restart{n,it}") ;
      }
                                                                                
      // Set the restart flag.
      virtual void compute(const sequence &seq) {
        doRestart=((*n+1)%(*restartFrequency)==0 && *iterationFinished) ;
      }
  } ;
  ...
  class DoRestart : public singleton_rule {
    ...
    public:
      ...
      DoRestart() {
        ...
        name_store("restart_freq{n,it}",restartFrequency) ;
        name_store("do_restart{n,it}",doRestart) ;
        input("$n{n},ncycle{n},$it{n,it},restart_freq{n,it}");
        output("do_restart{n,it}") ;
      }
      ...
      virtual void compute(const sequence &seq) {
        ...
        doRestart=((*nCycle)%(*restartFrequency)==0 && (*it)==0) ;
      }
  } ;
  ...
  // Rule to read the initial condition for temperature from a restart file.
  class TemperatureInitialConditionRegionsRestart : public pointwise_rule {
    private:
      const_blackbox<ICparsedInitRegion> icRegionInfo ;
      const_param<string> restartNum ;
    ...
    public:
      ...
      // Define input and output.
      TemperatureInitialConditionRegionsRestart() {
        name_store("icRegionInfo",icRegionInfo) ;
        name_store("restartNum",restartNum) ;
        name_store("cellcenter",cellCenter) ;
        name_store("restart::T_ic",T_ic) ;
        input("icRegionInfo,restartNum,cellcenter") ;
        output("restart::T_ic") ;
        constraint("restart,geom_cells") ;
        disable_threading() ;
      }
      ...
      void calculate(Entity cell) {
        ...
        T_ic[cell]=icRegionInfo[cell].defaultState.getTemperature() ;

        // Loop over regions, overwriting temperature with latest region value
        // if the cell center is in the region.
        for(size_t j=0;j<icRegionInfo[cell].fluidRegions.size();++j) {
          if(icRegionInfo[cell].fluidRegions[j].geomTestFunc->inGeomPt(cellCenter[cell])) {
            T_ic[cell]=icRegionInfo[cell].fluidRegions[j].regionState.getTemperature() ;
          }
        }
      }
  ...
  register_rule<TemperatureInitialConditionRegionsRestart>
    registerTemperatureInitialConditionRegionsRestart ;
  ...
  // Rule to read the initial condition for k from a restart file.
  class KInitialConditionRegionsRestart : public pointwise_rule {
    private:
      const_blackbox<ICparsedInitRegion> icRegionInfo ;
      const_param<string> restartNum ;
      const_store<vect3d> cellCenter ;
    ...
    public:
      ...
      // Define input and output.
      KInitialConditionRegionsRestart() {
        name_store("icRegionInfo",icRegionInfo) ;
        name_store("restartNum",restartNum) ;
        name_store("cellcenter",cellCenter) ;
        name_store("restart::k_ic",k_ic) ;
      ...
      void calculate(Entity cell) {
        ...
        // Set the default state.
        real omega ;
        icRegionInfo[cell].defaultState.get_k_omega(k_ic[cell],omega) ;

        // Loop over regions, overwriting temperature with latest region value
        // if the cell center is in the region.
        for(size_t j=0;j<icRegionInfo[cell].fluidRegions.size();++j) {
          if(icRegionInfo[cell].fluidRegions[j].geomTestFunc->inGeomPt(cellCenter[cell])) {
            icRegionInfo[cell].fluidRegions[j].regionState.get_k_omega(k_ic[cell],omega) ;
          }
        }
      }
      ...
      void compute(const sequence &seq) {
        ...
        if(test){
          ...
          if(Loci::MPI_rank==0) cout << "WARNING: No k restart file. "
            << "Using k initial condition value from .vars file." << endl ;
  ...
  register_rule<KInitialConditionRegionsRestart>
    registerKInitialConditionRegionsRestart ;
  ...
  // Rule to read the initial condition for omega from a restart file.
  class OmegaInitialConditionRegionsRestart : public pointwise_rule {
    private:
      const_blackbox<ICparsedInitRegion> icRegionInfo ;
      const_param<string> restartNum ;
      const_store<vect3d> cellCenter ;
    ...
    public:
      ...
      // Define input and output.
      OmegaInitialConditionRegionsRestart() {
        name_store("icRegionInfo",icRegionInfo) ;
        name_store("restartNum",restartNum) ;
        name_store("cellcenter",cellCenter) ;
        name_store("restart::omega_ic",omega_ic) ;
      ...
      void calculate(Entity cell) {
        ...
        // Set the default state.
        real k ;
        icRegionInfo[cell].defaultState.get_k_omega(k,omega_ic[cell]) ;

        // Loop over regions, overwriting temperature with latest region value
        // if the cell center is in the region.
        for(size_t j=0;j<icRegionInfo[cell].fluidRegions.size();++j) {
          if(icRegionInfo[cell].fluidRegions[j].geomTestFunc->inGeomPt(cellCenter[cell])) {
            icRegionInfo[cell].fluidRegions[j].regionState.get_k_omega(k,omega_ic[cell]) ;
          }
        }
      }

      // Read the omega values from the file restart/k.hdf5
      void compute(const sequence &seq) {
        ostringstream oss ; oss << "restart/omega_hdf5." << *restartNum ;
        string fileName=oss.str() ;
        ifstream test(fileName.c_str()) ;
        if(test){
          ...
          if(Loci::MPI_rank==0) cout << "WARNING: No omega restart file. "
            << "Using omega initial condition value from .vars file." << endl ;
  ...
  register_rule<OmegaInitialConditionRegionsRestart>
    registerOmegaInitialConditionRegionsRestart ;
  ...
  class WriteRestartData : public singleton_rule {
    ...
    public:
      ...
      WriteRestartData() {
        ...
        input("ncycle{n},stime{n},restart_modulo{n,it}") ;
        output("OUTPUT{n,it}") ;
  ...
  class RestartMassFlux : public pointwise_rule {
    ...
    public:
      ...
      RestartMassFlux() {
        ...
        name_store("restart_modulo{n,it}",restart_modulo) ;
        name_store("OUTPUT{n,it}",OUTPUT) ;
        input("ncycle{n},massFlux{n,it}") ;
        input("restart_modulo{n,it}") ;
        output("OUTPUT{n,it}") ;
        conditional("do_restart{n,it}") ;
        constraint("faces{n,it}") ;
      }
