...
//		name_store("boundaryPressure{n}",p_f) ;
  ...
  //      name_store("tauWall",tauWall) ;
        name_store("area{n,it-1}",area) ;
        name_store("boundaryForceCSD(X){n,it}",boundaryForce) ;
        name_store("ci",ci) ;
...
//		 input("boundaryPressure{n}") ;
  ...
  //      input("tauWall"); 
   //     input("ci->gradv3d(v){n,it},laminarViscosity_f{n,it},noWallFunction") ;
        output("boundaryForceCSD(X){n,it}") ;
        constraint("X{n,it}") ;
        disable_threading() ;
      }
  ...
  class AssembleRHSFSI : public pointwise_rule {
    ...
    public:
      ...
      AssembleRHSFSI() {
        ...
        name_store("boundaryForceCSD(X){n,it}", boundaryForceCSD) ;
        input("boundaryForceCSD(X){n,it}") ;
        output("FSI_B(X){n,it}") ;
        constraint("X{n,it}") ;
...
//         name_store("fsiNumBoundaryFace(X)",fsiNumBoundaryFace) ;
//         name_store("fsiBoundaryFaceToRow(X)",fsiBoundaryFaceToRow) ;
//         input("fsiNumBoundaryFace(X)") ;
//         output("fsiBoundaryFaceToRow(X)") ;
//         constraint("X") ;
