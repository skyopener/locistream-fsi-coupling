...
#line 366 "FSI_CSD2CFD.loci"
    Loci::blackbox<ublas::matrix<real,ublas::row_major> >  L_CSDRBFweights_nit__ ; 
#line 366 "FSI_CSD2CFD.loci"
public:
#line 366 "FSI_CSD2CFD.loci"
...
#line 366 "FSI_CSD2CFD.loci"
       name_store("CSDRBFweights{n,it}",L_CSDRBFweights_nit__) ;
#line 366 "FSI_CSD2CFD.loci"
       input("CSDnodes_ic,CSDdisplacementsStar{n,it},ncycle{n},FSICSD2CFDRBFr,FSICSD2CFDRBFa,FSICSD2CFDRBFnr,FSICSD2CFDRBFMaxLinearSolverIterations,FSICSD2CFDRBFTolerance,CFDIterationFinished{n,it-1}") ;
#line 366 "FSI_CSD2CFD.loci"
       output("CSDRBFweights{n,it}") ;
#line 366 "FSI_CSD2CFD.loci"
...
#line 366 "FSI_CSD2CFD.loci"
	...
	//	cout << "rank = " << rank << ", " << "Displacement Interpolation CSD -> CFD: Assembling RHS" << endl ;
		...
		for(int i=0;i<Nb;++i) {
			valuex=(*L_CSDdisplacementsStar_nit__)(i,0);
...
//			CSD2CFDRHS.close();
	...
	//	cout << "rank = " << rank << ", " << "Displacement Interpolation CSD -> CFD: system solved" << endl ;
		...
		(*L_CSDRBFweights_nit__).resize(Nbd1, 3) ;
		PetscScalar *phixCopy, *phiyCopy, *phizCopy ;
		FSI_phi.GetArray(&phixCopy,&phiyCopy,&phizCopy) ;
		for(int i=0;i<Nbd1;++i) {
			(*L_CSDRBFweights_nit__)(i,0) = phixCopy[i] ;
			(*L_CSDRBFweights_nit__)(i,1) = phiyCopy[i] ;
			(*L_CSDRBFweights_nit__)(i,2) = phizCopy[i] ;
		}
...
#line 513 "FSI_CSD2CFD.loci"
    Loci::const_param<bool>  L_CFDIterationFinished_nit_M_1__ ; 
#line 513 "FSI_CSD2CFD.loci"
    Loci::const_blackbox<ublas::matrix<real,ublas::row_major> >  L_CSDRBFweights_nit__ ; 
#line 513 "FSI_CSD2CFD.loci"
    Loci::const_store<vect3d>  L_pos_ ; 
#line 513 "FSI_CSD2CFD.loci"
    Loci::store<vect3d>  L_node_s_b_flex_nit__ ; 
#line 513 "FSI_CSD2CFD.loci"
public:
#line 513 "FSI_CSD2CFD.loci"
...
#line 513 "FSI_CSD2CFD.loci"
       name_store("node_s_b_flex{n,it}",L_node_s_b_flex_nit__) ;
#line 513 "FSI_CSD2CFD.loci"
       input("pos,CSDnodes_ic,CSDYorigin,ncycle{n},CSD2dSpanCenter,CSDdimension,CSDRBFweights{n,it},FSICSD2CFDRBFr,FSICSD2CFDRBFa,FSICSD2CFDRBFnr,CFDIterationFinished{n,it-1}") ;
#line 513 "FSI_CSD2CFD.loci"
       output("node_s_b_flex{n,it}") ;
#line 513 "FSI_CSD2CFD.loci"
...
#line 514 "FSI_CSD2CFD.loci"
	...
	if (L_CFDIterationFinished_nit_M_1__[_e_]) {
		...
		real distance =0. ;
		real r =(L_FSICSD2CFDRBFr_[_e_]), a =(L_FSICSD2CFDRBFa_[_e_]) ;
		int rbfNumber = (L_FSICSD2CFDRBFnr_[_e_]) ; // we get input as reals, convert to int
		int d = 3;
		...
		vector <real > value (Nb ,0.);  
		  for (int i =0;i <Nb ;++i ) {
			...
			for (int di =0;di <d ;++di ) {
				double polynom = (L_CSDRBFweights_nit__[_e_])(Nb , di ) ;
				polynom += (L_CSDRBFweights_nit__[_e_])(Nb +1,di ) * posx ;
				polynom += (L_CSDRBFweights_nit__[_e_])(Nb +2,di ) * posy ;				
				polynom += (L_CSDRBFweights_nit__[_e_])(Nb +3,di ) * posz ;
				if (di ==0) {
					(L_node_s_b_flex_nit__[_e_]).x = polynom ;
				} else if (di ==1) {
					(L_node_s_b_flex_nit__[_e_]).y = polynom ;
				} else if (di ==2) {
					(L_node_s_b_flex_nit__[_e_]).z = polynom ;
				} else {
			...
			// RBF contribution
			for (int i =0;i <Nb ;++i ){		
				(L_node_s_b_flex_nit__[_e_]).x += (L_CSDRBFweights_nit__[_e_])(i ,0) * value [i ] ;
				(L_node_s_b_flex_nit__[_e_]).y += (L_CSDRBFweights_nit__[_e_])(i ,1) * value [i ] ;
				(L_node_s_b_flex_nit__[_e_]).z += (L_CSDRBFweights_nit__[_e_])(i ,2) * value [i ] ;
			}
