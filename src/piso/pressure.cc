  ...
  class BoundaryPressureInterpolation : public pointwise_rule {
    ...
    public:
      ...
      BoundaryPressureInterpolation() {
        ...
        output("p_f") ;
        constraint("interface_BC,compressibleFlow") ;
      }
...
// Rules for marching the pressure.
  ...
  class IterationBuildPressure: public pointwise_rule {
    ...
    public:
      ...
      IterationBuildPressure() {
        name_store("pCorrected_p{n}",pCorrected) ;
        name_store("p{n,it=0}",p) ;
        input("pCorrected_p{n}") ;
        output("p{n,it=0}") ;
        constraint("geom_cells{n}") ;
      }

      // Set for a single cell.
      void calculate(Entity cell) { p[cell]=pCorrected[cell] ; }
  ...
  class IterationAdvancePressure : public pointwise_rule {
    ...
    public:
      ...
      IterationAdvancePressure() {
        name_store("pCorrected_c{n,it}",pCorrected) ;
        name_store("p{n,it+1}",p) ;
        input("pCorrected_c{n,it}") ;
        output("p{n,it+1}") ;
  ...
  // Iteration collapse rule for pressure.
