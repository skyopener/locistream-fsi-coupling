    ...
    public:
      ...
      virtual void compute(const sequence& seq) {
        if(*timeIntegrator=="BDF" || *timeIntegrator=="CN"){
          *timeIntegratorFactor0=1.0 ; *timeIntegratorFactor1=0.0 ;
        }else{
          if((*n)!=0){
            *timeIntegratorFactor0=2.0 ; *timeIntegratorFactor1=1.0 ;
          }else{
            *timeIntegratorFactor0=1.0 ; *timeIntegratorFactor1=0.0 ;
          }
        }
  ...
  class BoundaryVelocityFunctionUnsteadyCompute : public pointwise_rule {
    ...
    public:
      ...
      BoundaryVelocityFunctionUnsteadyCompute() {
        ...
        input("stime{n},timeStep{n},facecenter{n}") ;
...
//-----------------------------------------------------------------------------
// Boundary condition rules for corrected velocity in predictor and corrector
// stages.

  // Default boundary condition rule for predictor.
  class DefaultBoundaryVCorrectedPredictor : public pointwise_rule {
    private:
      const_store<vect3d> v_f ;
      store<vect3d> vCorrected_p_f ;
    public:

      // Define input and output.
      DefaultBoundaryVCorrectedPredictor() {
        name_store("v_f",v_f) ;
        name_store("vCorrected_p_f",vCorrected_p_f) ;
        input("v_f") ;
        output("vCorrected_p_f") ;
      }

      // Set for a single face.
      void calculate(Entity face) { vCorrected_p_f[face]=v_f[face] ; }

      // Loop over faces.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<DefaultBoundaryVCorrectedPredictor>
    registerDefaultBoundaryVCorrectedPredictor ;

  // Default boundary condition rule for corrector.
  class DefaultBoundaryVCorrectedCorrector : public pointwise_rule {
    private:
      const_store<vect3d> v_f ;
      store<vect3d> vCorrected_c_f ;
    public:

      // Define input and output.
      DefaultBoundaryVCorrectedCorrector() {
        name_store("v_f",v_f) ;
        name_store("vCorrected_c_f",vCorrected_c_f) ;
        input("v_f") ;
        output("vCorrected_c_f") ;
      }

      // Set for a single face.
      void calculate(Entity face) { vCorrected_c_f[face]=v_f[face] ; }

      // Loop over faces.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<DefaultBoundaryVCorrectedCorrector>
    registerDefaultBoundaryVCorrectedCorrector ;

//-----------------------------------------------------------------------------
// Rules to compute net outward mass flux from each cell. This is not really
// the net mass flux since we are now summing only over internal faces since
// these are the only faces that we need to change coefficients in order to get
// a diagonally dominant matrix. This rule has also been made parametric in
// order to support the PISO implementation.
...
//-----------------------------------------------------------------------------
// Boundary condition rules for vCorrected. The default rule handles all
// boundaries where vCorrected_f=v_f. Priority rules override for boundaries
// where the velocity is changing.

  // Default rule. For moving grid problems, the velocity on noslip and slip
  // boundaries is assigned to v_f by the gridMotion module and these values
  // will be used here.
  class BoundaryCorrectedVelocityDefault : public pointwise_rule {
    private:
      const_store<vect3d> v_f ;
      store<vect3d> vCorrected_f ;
    public:

      // Define input and output.
      BoundaryCorrectedVelocityDefault() {
        name_store("v_f",v_f) ;
        name_store("vCorrected_f(X,Y)",vCorrected_f) ;
        input("v_f") ;
        output("vCorrected_f(X,Y)") ;
        constraint("boundaryFaces") ;
      }

      // Set for a single face.
      void calculate(Entity face) { vCorrected_f[face]=v_f[face] ; }

      // Loop over faces.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<BoundaryCorrectedVelocityDefault>
    registerBoundaryCorrectedVelocityDefault ;

  // Rule for for assigning velocity on outlet faces. Right now we are using
  // a simple zero-gradient extrapolation. This rule does not allow
  // entrainment to occur.
  class BoundaryCorrectedVelocityOutlet : public pointwise_rule {
    private:
      const_Map ci ;
      const_store<Area> area ;
    ...
    public:
      ...
      // Define input and output.
      BoundaryCorrectedVelocityOutlet() {
        name_store("ci",ci) ;
        name_store("area",area) ;
        name_store("vCorrected(X,Y)",vCorrected) ;
        name_store("outlet::vCorrected_f(X,Y)",vCorrected_f) ;
        input("ci->vCorrected(X,Y),area") ;
        output("outlet::vCorrected_f(X,Y)") ;
        constraint("outlet_BC") ;
      }

      // Calculate face velocity for a single face.
      void calculate(Entity face) {
        vCorrected_f[face]=(dot(vCorrected[ci[face]],area[face].n)>=0.0)?
          vCorrected[ci[face]]:vect3d(0.0,0.0,0.0) ;
      }
  ...
  register_rule<BoundaryCorrectedVelocityOutlet>
    registerBoundaryCorrectedVelocityOutlet ;

  // Rule for for assigning corrected velocity on symmetry boundary faces.
  class BoundaryCorrectedVelocitySymmetry : public pointwise_rule {
    private:
      const_store<Area> area ;
      const_Map ci ;
      const_store<vect3d> vCorrected ;
    ...
    public:
      ...
      // Define input and output.
      BoundaryCorrectedVelocitySymmetry() {
        name_store("area",area) ;
        name_store("ci",ci) ;
        name_store("vCorrected(X,Y)",vCorrected) ;
        name_store("symmetry::vCorrected_f(X,Y)",vCorrected_f) ;
        input("area,ci->vCorrected(X,Y)") ;
        output("symmetry::vCorrected_f(X,Y)") ;
        constraint("symmetry_BC") ;
      }

      // Calculate face velocity for a single face.
      void calculate(Entity face) {
        const vect3d vCell=vCorrected[ci[face]] ;
        vCorrected_f[face]=vCell-dot(vCell,area[face].n)*area[face].n ;
      }
  ...
  register_rule<BoundaryCorrectedVelocitySymmetry>
    registerBoundaryCorrectedVelocitySymmetry ;
...
// Rules for computing eddy viscosity for laminar flows.
  ...
  class EddyViscosityLaminarFlowBoundary : public pointwise_rule {
    ...
    public:
      ...
      // Set eddy viscosity to zero for a sequence of faces.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;
...
//-----------------------------------------------------------------------------
// Rules to compute the hatted velocity and the starred hatted velocity that
// are used in the corrector stage.

  // Hatted velocity.
  class ComputeVHat : public pointwise_rule {
    private:
      const_store<real> vMainCoefficient ;
      const_store<vect3d> H_it,H ;
      store<vect3d> vHat ;
    public:
      ...
      ComputeVHat() {
        ...
        name_store("H_it",H_it) ;
        name_store("H(v)",H) ;
        name_store("vHat",vHat) ;
        input("vMainCoefficient,H_it,H(v)") ;
        output("vHat") ;
      ...
      void calculate(Entity cell) {
        vHat[cell]=(H[cell]-H_it[cell])/vMainCoefficient[cell] ;
      }
  ...
  register_rule<ComputeVHat> registerComputeVHat ;

  // Starred hatted velocity.
  class ComputeVStarHat : public pointwise_rule {
    private:
      const_store<real> vMainCoefficient_it,vMainCoefficient ;
      const_store<vect3d> v,vHat ;
      store<vect3d> vStarHat ;
    public:
      ...
      ComputeVStarHat() {
        ...
        name_store("v",v) ;
        name_store("vHat",vHat) ;
        name_store("vStarHat",vStarHat) ;
        input("vMainCoefficient_it,vMainCoefficient,v,vHat") ;
        output("vStarHat") ;
        constraint("geom_cells") ;
      }
      ...
      // Compute for a single cell.
      void calculate(Entity cell) {
        vStarHat[cell]=(vMainCoefficient_it[cell]/vMainCoefficient[cell])*
          v[cell]+vHat[cell] ;
      }
  ...
  register_rule<ComputeVStarHat> registerComputeVStarHat ;

//-----------------------------------------------------------------------------
// Rules for assembling the momentum equation.
  ...
  class DiagonalDominanceToVelocityMainCoefficientInterior : public
  apply_rule<store<real>,Loci::Summation<real> > {
    private:
      const_Map cl,cr ;
      const_param<real> thetaParameter ;
    ...
    public:
      ...
      // Define input and output.
      DiagonalDominanceToVelocityMainCoefficientInterior() {
        name_store("cl",cl) ;
        name_store("cr",cr) ;
        name_store("thetaParameter",thetaParameter) ;
        name_store("massFlux",massFlux) ;
  ...
  register_rule<DiagonalDominanceToVelocityMainCoefficientInterior>
    registerDiagonalDominanceToVelocityMainCoefficientInterior ;
  ...
  Loci::Summation<real> > {
    ...
    public:
      ...
      void calculate(Entity cell) {
        vMainCoefficient[cell]+=0.5*rho[cell]*vol[cell]*cellRadius[cell]*
  ...
  // dominant matrix.
  class DiagonalDominanceToVelocitySourceTerm : public apply_rule<store<vect3d>,
  Loci::Summation<vect3d> > {
    ...
    public:
      ...
      // Define input and output.
      DiagonalDominanceToVelocitySourceTerm() {
        name_store("v",v) ;
        name_store("netMassFlux(massFlux)",netMassFlux) ;
        name_store("vSourceTerm",vSourceTerm) ;
        input("v,netMassFlux(massFlux)") ;
        output("vSourceTerm") ;
      ...
      void calculate(Entity cell) {
        vSourceTerm[cell]-=netMassFlux[cell]*v[cell] ;
      }
  ...
  register_rule<DiagonalDominanceToVelocitySourceTerm>
    registerDiagonalDominanceToVelocitySourceTerm ;
  ...
  apply_rule<store<vect3d>,Loci::Summation<vect3d> > {
    ...
    public:
      ...
      void calculate(Entity face) {
        real netMassFlux=massFlux[face]-gridMassFlux[face] ;
        if(netMassFlux>0.0) vSourceTerm[ci[face]]-=netMassFlux*
  ...
  class VelocityTemporalSourceTermBDF : public pointwise_rule {
    private:
      const_param<real> timeStep ;
      const_store<real> timeStepFactor ;
      const_store<real> rho,vol,cellRadius ;
      const_store<vect3d> v ;
      store<vect3d> vTemporalSourceTerm ;
    public:

      // Define input and output.
      VelocityTemporalSourceTermBDF() {
        name_store("timeStep{n}",timeStep) ;
        ...
        name_store("vTemporalSourceTerm{n}",vTemporalSourceTerm) ;
        input("rho{n},v{n},vol{n},cellRadius{n}") ;
        input("timeStep{n},timeStepFactor{n}") ;
        output("vTemporalSourceTerm{n}") ;
        constraint("BDFIntegrator,geom_cells") ;
      }

      // Compute for a single cell.
      void calculate(Entity cell) {
        vTemporalSourceTerm[cell]=(rho[cell]*vol[cell]*cellRadius[cell]/
          ((*timeStep)*timeStepFactor[cell]))*v[cell] ;
      }
  ...
  register_rule<VelocityTemporalSourceTermBDF>
    registerVelocityTemporalSourceTermBDF ;
  ...
  // promote to {n,it} where it is also needed.
  class VelocityTemporalSourceTermBDF2 : public pointwise_rule {
    private:
      const_param<int> n ;
      const_param<real> timeStep ;
      const_store<real> timeStepFactor ;
    ...
    public:
      ...
      // Define input and output.
      VelocityTemporalSourceTermBDF2() {
        name_store("$n{n}",n) ;
        name_store("timeStep{n}",timeStep) ;
        name_store("timeStepFactor{n}",timeStepFactor) ;
...
//      name_store("vol{n-1}",volOld) ;
        ...
        name_store("cellRadius{n}",cellRadius) ;
        name_store("vTemporalSourceTerm{n}",vTemporalSourceTerm) ;
//      input("$n{n},rho{n-1},rho{n},v{n-1},v{n},vol{n-1},vol{n}") ;
        input("$n{n},rho{n-1},rho{n},v{n-1},v{n},vol{n}") ;
      ...
      void calculate(Entity cell) {
        if((*n)!=0){
          vTemporalSourceTerm[cell]=((2.0*rho[cell]*vol[cell])*v[cell]-(0.5*
            rhoOld[cell]*vol[cell])*vOld[cell])*cellRadius[cell]/
  ...
  register_rule<VelocityTemporalSourceTermBDF2>
    registerVelocityTemporalSourceTermBDF2 ;
  ...
  Loci::Summation<vect3d> > {
    private:
      const_store<vect3d> vTemporalSourceTerm ;
      store<vect3d> vSourceTerm ;
    public:

      // Define input and output.
      TemporalToVelocitySourceTerm() {
        name_store("vTemporalSourceTerm",vTemporalSourceTerm) ;
        name_store("vSourceTerm",vSourceTerm) ;
        input("vTemporalSourceTerm") ;
        output("vSourceTerm") ;
        constraint("geom_cells") ;
      }
      ...
      void calculate(Entity cell) {
        vSourceTerm[cell]+=vTemporalSourceTerm[cell] ;
      }
  ...
  class ComputeVelocityMatrixDiagonal : public pointwise_rule {
    ...
    public:
      ...
      ComputeVelocityMatrixDiagonal() {
        ...
        input("vMainCoefficient") ;
        output("vStar_D") ;
        constraint("geom_cells") ;
      }

      // Set value for a single cell.
      void calculate(Entity cell) { D[cell]=vMainCoefficient[cell] ; }
  ...
  // Initialize velocity RHS.
  class VelocityRHSUnit : public unit_rule {
    private:
      store<vect3d> B ;
    public:
      ...
      // Initialize to zero.
      void calculate(Entity cell) { B[cell]=vect3d(0.0,0.0,0.0) ; }

      // Loop over cells.
      virtual void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<VelocityRHSUnit> registerVelocityRHSUnit ;

  // Rule to add all source terms but pressure to RHS.
  class VSourceTermToVelocityRHS : public apply_rule<store<vect3d>,
  Loci::Summation<vect3d> > {
    private:
    ...
    public:
      ...
      // Define input and output.
      VSourceTermToVelocityRHS() {
        name_store("twoDimensionFactor",twoDimensionFactor) ;
        name_store("vSourceTerm",vSourceTerm) ;
        name_store("vStar_B",B) ;
        input("twoDimensionFactor,vSourceTerm") ;
        output("vStar_B") ;
        constraint("geom_cells") ;
      }
      ...
      // Add for a single cell.
      void calculate(Entity cell) {
        vect3d temp(vSourceTerm[cell]) ; temp.z*=(*twoDimensionFactor) ;
        B[cell]+=temp ;
      }
  ...
  register_rule<VSourceTermToVelocityRHS> registerVSourceTermToVelocityRHS ;

  // Rule to add the pressure contribution to the RHS for interior
  // faces.
  class PressureToVelocityRHSInterior : public apply_rule<store<vect3d>,
  Loci::Summation<vect3d> > {
    private:
      const_Map cl,cr ;
      const_param<real> twoDimensionFactor ;
    ...
    public:
      ...
      void calculate(Entity face) {
        ...
        pressureSource.z*=(*twoDimensionFactor) ;
        B[cl[face]]-=pressureSource ; B[cr[face]]+=pressureSource ;
      }
  ...
  register_rule<PressureToVelocityRHSInterior>
    registerPressureToVelocityRHSInterior ;
  ...
  // Rule to add the pressure contribution to the RHS for boundary faces.
  class PressureToVelocityRHSBoundary : public apply_rule<store<vect3d>,
  Loci::Summation<vect3d> > {
    private:
      const_Map ci ;
      const_param<real> twoDimensionFactor ;
  ...
  register_rule<PressureToVelocityRHSBoundary>
    registerPressureToVelocityRHSBoundary ;

  // Rule to copy vStar_B for periodic faces.
  class VelocityRHSPeriodic : public pointwise_rule {
    private:
      const_Map cl,cr,pmap ;
      store<vect3d> B ;
    public:
  ...
  register_rule<VelocityRHSPeriodic> registerVelocityRHSPeriodic ;
...
//-----------------------------------------------------------------------------
// Rules for tracking vMainCoefficient at the iteration level.

  // Iteration build rule.
  class IterationBuildVMainCoefficient : public pointwise_rule {
    private:
      const_store<real> vMainCoefficient ;
      store<real> vMainCoefficient_it ;
    public:

      // Define input and output.
      IterationBuildVMainCoefficient() {
        name_store("vMainCoefficient{n}",vMainCoefficient) ;
        name_store("vMainCoefficient_it{n,it=0}",vMainCoefficient_it) ;
        input("vMainCoefficient{n}") ;
        output("vMainCoefficient_it{n,it=0}") ;
        constraint("geom_cells{n}") ;
      }

      // Set for a single cell.
      void calculate(Entity cell) {
        vMainCoefficient_it[cell]=vMainCoefficient[cell] ;
      }
  ...
  register_rule<IterationBuildVMainCoefficient>
    registerIterationBuildVMainCoefficient ;

  // Iteration advance rule.
  class IterationAdvanceVMainCoefficient : public pointwise_rule {
    private:
      const_store<real> vMainCoefficient ;
      store<real> vMainCoefficient_it ;
    public:
      ...
      // Define input and output.
      IterationAdvanceVMainCoefficient() {
        name_store("vMainCoefficient{n,it}",vMainCoefficient) ;
        name_store("vMainCoefficient_it{n,it+1}",vMainCoefficient_it) ;
        input("vMainCoefficient{n,it}") ;
        output("vMainCoefficient_it{n,it+1}") ;
  ...
  register_rule<IterationAdvanceVMainCoefficient>
    registerIterationAdvanceVMainCoefficient ;

//-----------------------------------------------------------------------------
// Rules for computing and tracking the source term H at the iteration level.

  // Rule to compute the velocity source term including both the explicit and
  // implicit parts. This does not include the pressure terms. Note that this
  // variable is not used during the solution of the momentum equations, but
  // is only used in the correction stage for the pressure equation. This
  // variable is parametric, with X representing velocity.
  class VelocitySourceTermExplicitImplicit : public pointwise_rule {
    private:
      const_Map cl,cr ;
      const_multiMap upper,lower ;
      const_store<vect3d> v ;
      const_store<real> vStar_L,vStar_U ;
      const_store<vect3d> vSourceTerm ;
      store<vect3d> H ;
    public:

      // Define input and output.
      VelocitySourceTermExplicitImplicit() {
        name_store("cl",cl) ;
        name_store("cr",cr) ;
        name_store("upper",upper) ;
        name_store("lower",lower) ;
        name_store("X",v) ;
        name_store("vStar_L",vStar_L) ;
        name_store("vStar_U",vStar_U) ;
        name_store("vSourceTerm",vSourceTerm) ;
        name_store("H(X)",H) ;
        input("X,lower->vStar_L,upper->vStar_U") ;
        input("upper->cr->X,lower->cl->X,vSourceTerm") ;
        output("H(X)") ;
        constraint("geom_cells") ;
      }

      // Add explicit and implicit contributions for the cell.
      void calculate(Entity cell) {
        H[cell]=vSourceTerm[cell] ;
        for(const int *ui=upper.begin(cell);ui!=upper.end(cell);++ui)
          H[cell]-=vStar_U[*ui]*v[cr[*ui]] ;
        for(const int *li=lower.begin(cell);li!=lower.end(cell);++li)
          H[cell]-=vStar_L[*li]*v[cl[*li]] ;
      }
  ...
  register_rule<VelocitySourceTermExplicitImplicit>
    registerVelocitySourceTermExplicitImplicit ;

  // Iteration build rule.
  class IterationBuildH : public pointwise_rule {
    private:
      const_store<vect3d> H ;
      store<vect3d> H_it ;
    public:

      // Define input and output.
      IterationBuildH() {
        name_store("H(vStar){n}",H) ;
        name_store("H_it{n,it=0}",H_it) ;
        input("H(vStar){n}") ;
        output("H_it{n,it=0}") ;
        constraint("geom_cells{n}") ;
      }
      ...
      // Set for a single cell.
      void calculate(Entity cell) { H_it[cell]=H[cell] ; }

      // Loop over cells.
      void compute(const sequence &seq) { do_loop(seq,this) ; }
  } ;

  register_rule<IterationBuildH> registerIterationBuildH ;

  // Iteration advance rule.
  class IterationAdvanceH : public pointwise_rule {
    private:
      const_store<vect3d> H ;
      store<vect3d> H_it ;
    public:
      ...
      // Define input and output.
      IterationAdvanceH() {
        name_store("H(v){n,it}",H) ;
        name_store("H_it{n,it+1}",H_it) ;
        input("H(v){n,it}") ;
        output("H_it{n,it+1}") ;
        constraint("geom_cells{n,it}") ;
      }
  ...
  register_rule<IterationAdvanceH> registerIterationAdvanceH ;

//-----------------------------------------------------------------------------
// Rules for marching the velocity.
  ...
  class IterationBuildVelocity : public pointwise_rule {
    ...
    public:
      ...
      IterationBuildVelocity() {
        name_store("vCorrected_p{n}",vCorrected) ;
        name_store("v{n,it=0}",v) ;
        input("vCorrected_p{n}") ;
        output("v{n,it=0}") ;
        constraint("geom_cells{n}") ;
      }

      // Set for a single cell.
      void calculate(Entity cell) { v[cell]=vCorrected[cell] ; }
  ...
  class IterationAdvanceVelocity : public pointwise_rule {
    ...
    public:
      ...
      IterationAdvanceVelocity() {
        name_store("vCorrected_c{n,it}",vCorrected) ;
        name_store("v{n,it+1}",v) ;
        input("vCorrected_c{n,it}") ;
        output("v{n,it+1}") ;
        constraint("geom_cells{n,it}") ;
      }
  ...
  class CheckIterationFinished : public singleton_rule {
    private:
      ...
      const_param<ScalarResidual> pPrimeStarResidualData ;
...
//      input("vResidualData{n,it},pPrimeResidualData{n,it}") ;
        input("pPrimeStarResidualData{n,it}") ;
...
//      constraint("vResidualData{n,it}") ;
      ...
      void compute(const sequence &seq) {
        ...
        if(Loci::MPI_rank==0){
          cout << "R: " << *nCycle << " " << *it << " " ;
          cout << pPrimeStarResidualData->totalResidual ;
