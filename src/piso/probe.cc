  ...
  class OptionalProbe : public optional_rule {
    ...
    public:
      OptionalProbe() {
        ...
        output("probe") ;
        comments("Put probes into the solution.  The user specifies a list of") ;
        comments("probes in the form of <probe1=[x1,y1,z1],probe2=[x2,y2,z2],...>") ;
        comments("and the solver produces probex.dat files that contains the") ;
        comments("solution located at that point over time.") ;
...
namespace streamUns {
  ...
  class DoProbe : public singleton_rule {
    ...
    public:
      ...
      DoProbe() {
        ...
        name_store("probe_freq{n}",probeFrequency) ;
        name_store("do_probe{n}",doProbe) ;
        input("ncycle{n}") ;
        input("probe_freq{n}") ;
        output("do_probe{n}") ;
      }
      ...
      // Set the value.
      virtual void compute(const sequence &seq) {
        doProbe=(*nCycle % *probeFrequency==0) ;
      }
  } ;
  ...
  class OutputProbe : public pointwise_rule {
    ...
    public:
      ...
      OutputProbe() {
        name_store("ncycle{n}",nCycle) ;
        name_store("probe{n}",probe) ;
        name_store("probeList{n}",probeList) ;
        name_store("stime{n}",solutionTime) ;
        name_store("numSpecies{n}",numSpecies) ;
        name_store("OUTPUT{n}",OUTPUT) ;
        input("ncycle{n},probe{n},probeList{n},stime{n},numSpecies{n}") ;
        output("OUTPUT{n}") ;
        conditional("do_probe{n}") ;
      }
      ...
      void compute(const sequence &seq) {
        if(Loci::MPI_rank==0){
          ...
          li!=l.end();++li,++i) {
            ...
            out << *nCycle << " " << *solutionTime << " "
              << (*probeList)[i].density << " " << (*probeList)[i].velocity
              << " " << (*probeList)[i].pressure << " "
